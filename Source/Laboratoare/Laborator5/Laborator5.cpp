...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	// 2 obiecte random
	{
		...
		modelMatrix *= glm::rotate(glm::mat4(1), 45.0f, glm::vec3(0, 0, 1));
		modelMatrix *= glm::rotate(glm::mat4(1), 45.0f, glm::vec3(0, 1, 0));
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}

	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 3, 0));
		modelMatrix *= glm::rotate(glm::mat4(1), 20.0f, glm::vec3(0, 1, 0));
		modelMatrix *= glm::rotate(glm::mat4(1), 83.0f, glm::vec3(1, 0, 0));
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_O)) {
			projectionMatrix = glm::ortho(-orthSize, orthSize, -orthSize, orthSize, 0.5f, 200.0f);
			isOrth = true;
		}
		if (window->KeyHold(GLFW_KEY_1)) {
			if (isOrth) {
				orthSize -= deltaTime * 2.0f;
				projectionMatrix = glm::ortho(-orthSize, orthSize, -orthSize, orthSize, 0.5f, 200.0f);
			}
			else {
				fov -= deltaTime * 20.0f;
