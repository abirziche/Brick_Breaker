...
void Laborator6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
	...
	int timeSinceStartPosition = glGetUniformLocation(shader->program, "timeSinceStart");
	float timeSinceStart = Engine::GetElapsedTime();
	glUniform1f(timeSinceStartPosition, timeSinceStart);
