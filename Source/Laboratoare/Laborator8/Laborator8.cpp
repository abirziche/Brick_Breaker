...
void Laborator8::Update(float deltaTimeSeconds)
{
...

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1,1,0));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int loc = glGetUniformLocation(shader->program, "angle");
	glUniform1f(loc, angle);

	int poss = glGetUniformLocation(shader->program, "t");
	glUniform1f(poss, typeOfLight);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_UP)) angleOX += deltaTime;
	if (window->KeyHold(GLFW_KEY_RIGHT)) angleOZ += deltaTime;
	if (window->KeyHold(GLFW_KEY_DOWN)) angleOX -= deltaTime;
	if (window->KeyHold(GLFW_KEY_LEFT)) angleOZ -= deltaTime;
	...
	lightDirection = glm::vec3(0, -1, 0);
	lightDirection = matrix * glm::vec4(lightDirection, 0);
}
