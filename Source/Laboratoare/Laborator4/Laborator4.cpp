...
void Laborator4::Init()
{
	...
	// Also help with the original angular step
	timeSinceStart = 0;
}
...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform3D::RotateOY(-timeSinceStart / 10.f);
	modelMatrix *= Transform3D::Scale(3, 3, 3);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);

	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(0, -10, 0);
	modelMatrix *= Transform3D::RotateOY(timeSinceStart / .4f);
	modelMatrix *= Transform3D::Translate(7, 0, 7);
	modelMatrix *= Transform3D::RotateOY(-timeSinceStart * 10);
	...
	modelMatrix *= Transform3D::RotateOY(timeSinceStart / .4f);
	modelMatrix *= Transform3D::Translate(7, 0, 7);
	modelMatrix *= Transform3D::RotateOY(timeSinceStart * 16);
	modelMatrix *= Transform3D::Translate(2, 0, 2);
...
void Laborator4::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_1)) {
		scaleX *= expf(deltaTime);
		scaleY *= expf(deltaTime);
		scaleZ *= expf(deltaTime);
	}
